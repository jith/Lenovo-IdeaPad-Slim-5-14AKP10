...
| 9b | Upper-bass lift | `s9blift_*` | builtin `bq_peaking` | 200 Hz, Q 1.2, **Gain +6 dB** | **active** — added 4 Sep 2026 at 0 dB, taken to +3 on an A/B and to +6 on a second. Aimed at a measured 14 dB gap against an iPhone 13 at 160–250 Hz, the band a phone's bass actually lives in. Sits **before** GOTT, unlike 10b/10c: the compression it provokes pulls down the 25–63 Hz content that moves the cone, so displacement *falls* as this rises | — |
...
| 13 | A/B trim | `s13trim_*` | builtin `linear` | static gain from the loudness match | **unity** — tuned deliberately left hot, by **6.78 LU** as re-measured at `g_out` 6.50 / `Gain 2` 0.45 / stage 9b +6 | ITU-R BS.1770 |
...
| directory | chain |
|---|---|
| `tests/captures/` | **current** — `Gain 2` 0.45, `g_out` 6.50, stage 9b **+6 dB** |
| `gout650-lift3/` | the same at stage 9b +3 dB |
